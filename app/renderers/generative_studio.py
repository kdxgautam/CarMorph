...
STYLE_PROMPTS = {
    "light_studio": "Use a soft gray and white studio with clean high-key lighting.",
...
def build_studio_prompt(request: StudioRenderRequest) -> str:
    return (
...
class GenerativeStudioRenderer:
    ...
    def render(
        ...
        *,
        ...
        prompt = build_studio_prompt(modification)
        ...
        if modification.preserve_plate:
            ...
            if plate is not None and np.any(plate >= 128):
                result = _restore_masked_region(result, original, plate)
            else:
        ...
        try:
            ...
            (work / "provider.json").write_text(
                json.dumps({"provider": self.provider.name, "model_id": self.provider.model_id, "renderer_version": self.version}, indent=2),
                encoding="utf-8",
            )
